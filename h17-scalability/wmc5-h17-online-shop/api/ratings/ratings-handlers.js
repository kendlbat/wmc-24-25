...
const getAll = async (req, resp) => {
    try {
        ...
        let resultSet = await Rating.find({ productId: productId }).sort({ timestamp: 1 });
        resp.status(200);
        resp.json(resultSet);
    }
    catch (err) {
...
const getSummary = async (req, resp) => {
    try {
        let productId = req.params.productId;
        logger.debug(`Ratings - Fetching summary of ratings for productId`, productId);
...
const deleteById = async (req, resp) => {
    ...
    try {
        ...
        if (opResult.deletedCount == 1) {
            ...
            resp.status(404).type('text/plain').send(`Rating with id ${ratingId} not found`);
        }
    }
    catch (err) {
